    ...
    <ul class="sider">
      ...
      <li class="item">
        <input
          type="checkbox"
          name="legend-flip"
          id="legend-poi_site_water_ground"
          v-model="legendSelected.poi_site_water_ground"
          @change="showSpecPOI('poi_site_water_ground')"
        />
        <label for="legend-poi_site_water_ground">地下水水位站</label>
      </li>
      <li class="item">
        <input
          type="checkbox"
          name="legend-flip"
          id="legend-poi_site_meteorological"
          v-model="legendSelected.poi_site_meteorological"
          @change="showSpecPOI('poi_site_meteorological')"
        />
        <label for="legend-poi_site_meteorological">气象站</label>
      </li>
      <li class="item">
        <input
          type="checkbox"
          name="legend-flip"
          id="legend-poi_site_water_leve"
          v-model="legendSelected.poi_site_water_leve"
          @change="showSpecPOI('poi_site_water_leve')"
        />
        <label for="legend-poi_site_water_leve">水位站</label>
      </li>
      <li class="item">
        <input
          type="checkbox"
          name="legend-flip"
          id="legend-poi_site_water_hydrological"
          v-model="legendSelected.poi_site_water_hydrological"
          @change="showSpecPOI('poi_site_water_hydrological')"
        />
        <label for="legend-poi_site_water_hydrological">水文站</label>
      </li>
      <li class="item">
        <input
          type="checkbox"
          name="legend-flip"
          id="legend-poi_site_water_quality"
          v-model="legendSelected.poi_site_water_quality"
          @change="showSpecPOI('poi_site_water_quality')"
        />
        <label for="legend-poi_site_water_quality">水质站</label>
      </li>
      <li class="item">
        <input
          type="checkbox"
          name="legend-flip"
          id="legend-poi_uav"
          v-model="legendSelected.poi_uav"
          @change="showSpecPOI('poi_uav')"
        />
        <label for="legend-poi_uav">无人机</label>
      </li>
      <li class="item">
        <input
          type="checkbox"
          name="legend-flip"
          id="legend-poi_uav_hangar"
          v-model="legendSelected.poi_uav_hangar"
          @change="showSpecPOI('poi_uav_hangar')"
        />
        <label for="legend-poi_uav_hangar">无人机库</label>
      </li>
      <li class="item">
        <input
          type="checkbox"
          name="legend-flip"
          id="legend-poi_site_rain"
          v-model="legendSelected.poi_site_rain"
          @change="showSpecPOI('poi_site_rain')"
        />
        <label for="legend-poi_site_rain">雨量站</label>
      </li>
      <li class="item">
        <input
          type="checkbox"
          name="legend-flip"
          id="legend-poi_executor"
          v-model="legendSelected.poi_executor"
          @change="showSpecPOI('poi_executor')"
        />
        <label for="legend-poi_executor">执法人员</label>
      </li>
      <li class="item">
        <input
          type="checkbox"
          name="legend-flip"
          id="legend-camera"
          v-model="legendSelected.camera"
          @change="showSpecPOI('camera')"
        />
        <label for="legend-camera">摄像头</label>
      </li>
    </ul>
    <ul class="controller">
      <li class="item">
...
import poiAllList from "../data/poi-all.json";
...
export default {
  ...
  data() {
    return {
      ...
      qlSelected: false,
      legendSelected: {
        poi_site_water_ground: false,
        poi_site_meteorological: false,
        poi_site_water_leve: false,
        poi_site_water_hydrological: false,
        poi_site_water_quality: false,
        poi_uav: false,
        poi_uav_hangar: false,
        poi_site_rain: false,
        poi_executor: false,
        camera: false
      }
  ...
  methods: {
    showAllPOI() {
      ...
      points = poiAllList.map(o => {
        ...
        this.legendSelected[o.Type] = true;
        return node;
      });
    ...
    showSpecPOI(type) {
      //this.clearAllElement();
      let specs = _.filter(poiAllList, { Type: type });
      let node = null;

      if (!this.legendSelected[type]) {
        //Delete
        specs.map(o=>{
          this.renderer.SuperAPI('RemovePOI',o.Id);
        })
      } else {
        specs.map(o => {
          node = {
    ...
    requestCloudRenderer() {
      axios({
        ...
        let jsData = res.data;
        this.requestUrlByToken(jsData.token);
      });
    },
    requestLocalHost() {
      axios
        .get("https://localhost:5001/superAPI/GetToken", {
          params: {
            username: this.username,
            pwd: this.pwd
          }
    ...
    requestUrlByToken(token) {
      axios
        .get(this.serverUrl + "Renderers/Any/" + this.projectId, {
          headers: {
    ...
    lifeCycleHooks(data) {
      ...
      switch (json.func_name) {
        ...
        case "APIAlready":
          ...
          break;
        case "OnPOIClick":
          let onPOIClick_id = json.args.id;
          let data = this.POIHash[onPOIClick_id];
          this.$emit("on-poi-click", data);
          break;
        case "OnPOIMouseOverlap":
          let onPOIHover_id = json.args.id;
          this.$emit("on-poi-hover", onPOIHover_id);
          break;
        case "OnCameraInfo":
          let camera_coord = json.args;
          //数据写入组件仓库
    ...
    renderLineDrew(data) {
      if (this.brushMode === "line") {
        let pathPoints = data.points.map(o => {
          return {
            coord: o.coord,
            coord_z: 100
          };
        ...
        this.autoIndex.path++;
        let liveID = "path_" + this.autoIndex.path;
        let pathData = {
          object_id: liveID,
          coord_type: 0,
          style_type: 2,
    ...
    renderAreaDrew(data) {
      if (this.brushMode === "area") {
        let rangePoints = data.points.map(o => {
          return {
            coord: o.coord,
            coord_z: 2100
          };
        ...
        this.autoIndex.area++;
        let liveID = "area_" + this.autoIndex.area;
        let rangedata = {
    ...
    loadAllPOI() {
      pois.forEach(poi => {
        console.log(poi);
        let coord = this.culCoord(poi.coord);
        let obj = {
          object_id: poi.id,
    ...
    loadUAVAction() {
      this.clearAllElement();
      let p = uav[0];
      let start_coord = p.start_coord;
      let target_coord = p.target_coord;
      let effectdata = {
      ...
      this.renderer.SuperAPI("FocusMigrationMap", "uav_001", 5500);
    },
    loadStaffAction() {
      this.clearAllElement();
      let p = uav[0];
    ...
    createLakeArea(id, data, color) {
      ...
      let rangedata = {
        ...
        width: "50000.0", //区域底边宽度(米)
        rangehight: "500000", //区域围栏高度 (米)
        fillarea: "true", //是否填充区域底部
        points: dots
      };
      this.renderer.SuperAPI("AddRange", rangedata);
