<template>
  <div class="palel-point-pollution">
    <panel-title :title="title"></panel-title>
    <div class="chart" id="waterQualityPointPollution"></div>
  </div>
</template>

<script>
import PanelTitle from '@/components/Dashboard/PanelTitle'
import { PollutionPointConcentrationPreviousYear } from '@/api/waterQuality'
...
export default {
  ...
  methods: {
    renderChart () {
      ...
      PollutionPointConcentrationPreviousYear(param).then(res => {
        if (res) {
          ...
          this.$chartRender('waterQualityPointPollution', this.option)
        }
      })
    }
  },
